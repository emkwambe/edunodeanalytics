# EduNode Analytics - Developer Checklist v8.0
...
| Section | Change |
|---------|--------|
| B3 | CSV Import System COMPLETE - students, attendance, assessments |
| C4 | Risk Config Admin UI COMPLETE - `/[school_slug]/settings/risk-model` |
| E4 | Dashboard dosage widgets COMPLETE |
| E4.5 | NEW: Dosage summary component with compliance metrics |
| E4.6 | NEW: Dosage alerts component with inference rule display |
| G3 | Dosage widgets integrated into early-warning page |
| J | Sprint 5.2 (Dosage UI) marked COMPLETE |
| J | Sprint 5.3 (CSV Import, Admin UI) marked COMPLETE |
| Go/No-Go | CSV import gate PASS - Pilot Readiness now 11/12 (92%) |
...
- [ ] **BLOCKER:** `database.types.ts` must be regenerated via Supabase CLI
  - Current file is stale (missing 7 tables: 5 risk + 2 dosage)
  - Supabase CLI auth required: `npx supabase login` then `npx supabase gen types`
  - Once regenerated: remove `@ts-nocheck` and type assertions
...
- [x] Dosage engine uses type assertions for new tables (temporary workaround)
...
### B3. CSV Import System (SPRINT 5.3 - COMPLETE)
- [x] **CSV import API:** `POST /api/schools/[schoolId]/import`
  - Students import with upsert on `sis_student_id`
  - Attendance import updates `attendance_rate`, `days_present`, `days_absent`
  - Assessment import updates scores and growth percentiles
  - Admin-only access validation
  - Schema validation with row-level error reporting
  - Audit logging to `audit_logs` table
  - Auto-triggers risk evaluation after import
- [x] **CSV import UI:** `/[school_slug]/settings/import`
  - File upload with drag-and-drop
  - Three import types: Students, Attendance, Assessments
  - Client-side CSV parsing with Papaparse
  - Preview first 10 rows before confirm
  - Validation errors displayed inline
  - Import result with risk analysis summary
- [x] Navigation link in settings sidebar
- [x] Idempotent imports (re-upload updates, doesn't duplicate)
- [x] Max 5000 rows per import
...
### C4. Configurability (SPRINT 5.3 - COMPLETE)
...
- [x] **Admin UI:** `/[school_slug]/settings/risk-model`
  - Weight sliders for 5 indicators (must sum to 100%)
  - Threshold inputs for 3 tier boundaries
  - Indicator parameters (attendance floor, assessment floor, etc.)
  - Live preview with sample student calculation
  - Reset to defaults button
  - Save/validation with feedback
...
### E4. Dosage Analysis (SPRINT 5 - COMPLETE)

**Database:**
- [x] `intervention_sessions` table - individual session records
- [x] `intervention_dosage_metrics` table - aggregated metrics per intervention
- [x] `active_dosage_overview` view - dashboard data
- [x] `dosage_compliance_by_type` view - compliance aggregation
- [x] RLS policies for school-scoped access
- [x] Migration: `supabase/migrations/00007_intervention_dosage_metrics.sql`

**Core Engine:**
- [x] `src/lib/dosage/types.ts` - 280 lines of type definitions
- [x] `src/lib/dosage/dosage-analyzer.ts` - 620 lines
  - `DosageAnalyzer` class with factory `createDosageAnalyzer(schoolId)`
  - `analyzeIntervention()` - single intervention analysis
  - `analyzeSchool()` - school-wide summary
  - `computeMetrics()` - aggregate from sessions
  - `logSession()` - session completion logging
  - `generateSchedule()` - automatic schedule generation
- [x] `src/lib/dosage/index.ts` - module exports

**10 Inference Rules:**
1. [x] `chronic_no_show` - 3+ no-shows in last 2 weeks (critical)
2. [x] `declining_attendance` - Attendance dropped >20% from baseline (warning)
3. [x] `low_fidelity` - Average fidelity score below 70% (warning)
4. [x] `declining_fidelity` - Fidelity declining over 3+ sessions (warning)
5. [x] `behind_schedule` - 2-3 sessions behind planned pace (warning)
6. [x] `critically_behind` - 4+ sessions behind or <50% compliance (critical)
7. [x] `low_engagement` - Student engagement rate below 60% (warning)
8. [x] `cancelled_streak` - 3+ consecutive cancelled sessions (warning)
9. [x] `missing_sessions` - No sessions logged in 2+ weeks (warning)
10. [x] `dosage_gap` - Gap of 10+ days between sessions (info)

**API Endpoints:**
- [x] `GET /api/schools/[schoolId]/dosage` - school-wide summary
- [x] `GET /api/schools/[schoolId]/dosage/[interventionId]` - intervention analysis
- [x] `POST /api/schools/[schoolId]/dosage/[interventionId]` - recompute metrics
- [x] `GET /api/schools/[schoolId]/dosage/[interventionId]/sessions` - list sessions
- [x] `POST /api/schools/[schoolId]/dosage/[interventionId]/sessions` - create sessions
- [x] `GET /api/schools/[schoolId]/dosage/[interventionId]/sessions/[sessionId]` - session detail
- [x] `PATCH /api/schools/[schoolId]/dosage/[interventionId]/sessions/[sessionId]` - log session
- [x] `DELETE /api/schools/[schoolId]/dosage/[interventionId]/sessions/[sessionId]` - delete scheduled

**Dashboard Widgets (SPRINT 5.2 - COMPLETE):**
- [x] `src/lib/hooks/use-dosage.ts` - React hooks
  - `useDosageSummary(schoolId)` - school-wide dosage data
  - `useInterventionDosage(schoolId, interventionId)` - single intervention
  - `useDosageAlerts(schoolId)` - flattened alert list from flags
- [x] `src/components/dashboard/dosage-summary.tsx`
  - Total active interventions
  - Average compliance % with color coding (green >80%, amber 60-80%, red <60%)
  - Average fidelity %
  - Top 5 interventions with dosage issues
  - Links to intervention detail
- [x] `src/components/dashboard/dosage-alerts.tsx`
  - Shows alerts from inference rules
  - Badge severity: critical = red, warning = amber, info = blue
  - Rule labels for all 10 rules
  - Click navigates to intervention
- [x] Integrated into early-warning page

**Remaining:**
- [ ] Session logging UI (dedicated page)
- [ ] Fidelity checklist builder
...
- [x] CSV imports logged to audit_logs
...
### G3. Early Warning Dashboard Page (SPRINT 3-5 - COMPLETE)
...
**Dosage Components (SPRINT 5.2):**
- [x] `DosageSummary` - compliance overview with top issues
- [x] `DosageAlerts` - inference rule alerts
...
- [x] `useDosageSummary(schoolId)` - `src/lib/hooks/use-dosage.ts`
- [x] `useDosageAlerts(schoolId)` - `src/lib/hooks/use-dosage.ts`
...
- [x] **Are interventions being delivered?** - dosage compliance
...
### H3. Dosage Engine Tests (SPRINT 5 - COMPLETE)
- [x] Unit tests for all 10 inference rules (`dosage-analyzer.test.ts`)
- [x] Rule collection validation tests
- [x] 32 tests covering chronic no-show, declining attendance, fidelity, engagement, scheduling gaps

**Test Coverage:** 463 tests passing across 18 test files

### H4. Demo and Seed Data (SPRINT 3 - COMPLETE)
...
**Summary:**
- 51 items IMPLEMENTED (+7 from v7)
- 8 items PARTIAL (-2 from v7)
- 24 items MISSING (-3 from v7)
- 0 CONFLICTS between documents

**Priority Gaps Remaining:**
1. database.types.ts regeneration (BLOCKER)
2. Session logging UI
3. Demo walkthrough flow
...
| 5.1 | Dosage Foundation | **COMPLETE** | 2 tables, dosage-analyzer, 10 rules, 8 API endpoints, 32 tests |
| 5.2 | Dosage UI | **COMPLETE** | Dashboard widgets (dosage-summary, dosage-alerts), use-dosage hook |
| 5.3 | CSV Import & Admin UI | **COMPLETE** | CSV import (3 types), risk config admin page, risk pipeline integration |
...
| Dosage analysis computed | **PASS** | 5.1 | None |
| CSV import for schools without Clever/ClassLink | **PASS** | 5.3 | None |
| database.types.ts regenerated | **BLOCKED** | - | Supabase CLI |

**Pilot Readiness Score: 11/12 gates PASS (92%)**
...
### Sprint 5 (2 new tables)
`intervention_sessions`, `intervention_dosage_metrics`

Views: `active_dosage_overview`, `dosage_compliance_by_type`
...
### Dosage Engine (Sprint 5)
| File | Lines | Status |
|------|-------|--------|
| `src/lib/dosage/types.ts` | 280 | COMPLETE |
| `src/lib/dosage/dosage-analyzer.ts` | 620 | COMPLETE |
| `src/lib/dosage/index.ts` | 65 | COMPLETE |
| `src/lib/hooks/use-dosage.ts` | 170 | NEW |
| `supabase/migrations/00007_intervention_dosage_metrics.sql` | 185 | COMPLETE |

### Dashboard Components (Sprint 5.2)
| File | Lines | Status |
|------|-------|--------|
| `src/components/dashboard/dosage-summary.tsx` | 220 | NEW |
| `src/components/dashboard/dosage-alerts.tsx` | 200 | NEW |

### Settings Pages (Sprint 5.3)
| File | Lines | Status |
|------|-------|--------|
| `src/app/[school_slug]/settings/import/page.tsx` | 700 | NEW |
| `src/app/[school_slug]/settings/risk-model/page.tsx` | 580 | NEW |

### Import API (Sprint 5.3)
| Route | Method | Status |
|-------|--------|--------|
| `/api/schools/[schoolId]/import` | POST | NEW |

### Dosage API Routes (Sprint 5)
| Route | Method | Status |
|-------|--------|--------|
| `/api/schools/[schoolId]/dosage` | GET | COMPLETE |
| `/api/schools/[schoolId]/dosage/[interventionId]` | GET, POST | COMPLETE |
| `/api/schools/[schoolId]/dosage/[interventionId]/sessions` | GET, POST | COMPLETE |
| `/api/schools/[schoolId]/dosage/[interventionId]/sessions/[sessionId]` | GET, PATCH, DELETE | COMPLETE |
...
| `/api/schools/[schoolId]/export/risk-evaluations` | GET | COMPLETE |
...
| `src/lib/risk-engine/__tests__/metrics-aggregator.test.ts` | 33 | COMPLETE |
| `src/lib/risk-engine/__tests__/risk-system.test.ts` | 27 | COMPLETE |
| `src/lib/dosage/__tests__/dosage-analyzer.test.ts` | 32 | COMPLETE |

---

*Document Version: 8.0*
*Previous: v7.0 (March 7, 2026)*
