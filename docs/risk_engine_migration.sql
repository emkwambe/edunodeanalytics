...
--   9. Seed default configs for existing schools
--  10. Bootstrap student_metrics from existing students
--
-- Integrates with existing:
--   - schools, students, interventions, users, school_memberships
--   - public.risk_level enum (on_track, at_risk, critical)
-- ============================================================


-- 0. Add 'watch' to risk_level enum
DO $$
BEGIN
  IF NOT EXISTS (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_risk_config_active_school 
  ON public.risk_model_configs(school_id) WHERE is_active = true;
...
CREATE TABLE IF NOT EXISTS public.risk_alerts (
  ...
  alert_type        TEXT NOT NULL CHECK (
    alert_type IN (
      'threshold_breach', 'rapid_decline', 'chronic_absence',
      'intervention_overdue', 'new_risk_detected', 'trend_warning',
      'attendance_drop', 'grade_decline', 'consecutive_absences'
    )
  ),
...
-- 11. BOOTSTRAP STUDENT_METRICS FROM EXISTING STUDENTS
...
-- ============================================================
-- MIGRATION COMPLETE
-- Tables: 5 created, 1 view, 1 function, 15 RLS policies
-- Indexes: 11 created
-- Seeds: configs + student_metrics bootstrapped
-- ============================================================
